...
export const ContentProvider = ({ children }) => {
  ...
  const getAllData = async (page) => {
    ...
    setError(null);
    try {
      const result = await request("POST", "Proyecto/all", {
        pageNumber: page,
        pageSize: pageSize,
      });
      setAllData(result.data.Proyectos);
  ...
  const createUpdate = async (data) => {
    ...
    try {
      const result = await request(
        `${oneData?.id ? "PUT" : "POST"}`,
        `Proyecto/${oneData?.id ? "update" : "create"}`,
        {
          id: oneData?.id,
          nombre: data?.nombre,
      ...
      showNotification(
        `Proyecto ${oneData?.id ? "actualizado" : "creado"} exitosamente.`,
        "success"
      );
    } catch (error) {
      setError(error.message);
  ...
  const dropItem = async (data) => {
    ...
    setError(null);
    try {
      const result = await request(`DELETE`, `Proyecto/delete`, {
        id: data?.id,
      });
  ...
  const getOneData = async (data, opcion) => {
    ...
    setError(null);
    try {
      const result = await request(`POST`, "Proyecto/one", {
        id: data?.id,
      });
