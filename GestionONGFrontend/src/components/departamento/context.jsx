...
export const ContentProvider = ({ children }) => {
  ...
  const getAllData = async (page) => {
    ...
    setError(null);
    try {
      const result = await request("POST", "Departamento/all", {
        pageNumber: page,
        pageSize: pageSize,
      });
      setAllData(result.data.Departamentos);
  ...
  const createUpdate = async (data) => {
    ...
    try {
      const result = await request(
        `${oneData?.id ? "PUT" : "POST"}`,
        `Departamento/${oneData?.id ? "update" : "create"}`,
        {
          id: data?.id,
          nombre: data?.nombre,
        }
      ...
      showNotification(
        `Departamento ${oneData?.id ? "actualizado" : "creado"} exitosamente.`,
        "success"
      );
    } catch (error) {
      setError(error.message);
  ...
  const dropItem = async (data) => {
    ...
    setError(null);
    try {
      const result = await request(`DELETE`, `Departamento/delete`, {
        id: data?.id,
      });
      showNotification(
  ...
  const getOneData = async (data, opcion) => {
    ...
    setError(null);
    try {
      const result = await request(`POST`, "Departamento/one", {
        id: data?.id,
      });
